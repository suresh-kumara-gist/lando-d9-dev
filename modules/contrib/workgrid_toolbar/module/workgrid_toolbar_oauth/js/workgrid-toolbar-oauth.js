...
(function ($, Drupal, drupalSettings) {
    ...
    function toolbarauth() {
        return fetch('/workgrid-toolbar-oauth/get-toolbar-token', { headers: { Accept: 'application/json' } })
            .then(resp => {
                if (resp.ok) {
                    return resp.text();
                } else {
        ...
        attach: function (context, settings) {
            $('main', context).once('workgridToolbar').each(function () {
                workgrid.addEventListener('layout', function (event) {
                    var margintop = drupalSettings.workgrid_toolbar_oauth.margintop;
                    var marginbottom = drupalSettings.workgrid_toolbar_oauth.marginbottom;
                ...
                var spaceId = drupalSettings.workgrid_toolbar_oauth.spaceId;
                var companyCode = drupalSettings.workgrid_toolbar_oauth.companyCode;
